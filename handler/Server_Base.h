...
std::vector<std::pair<int, int>> find_item_list;
long long int daily_current_time = 0, daily_wait_time = 0, quest = 0, captcha = 0, lastIPWait = 0;
int HowManyGiftedGrowch = 0, day_item = 1796, day_count = 1;
...
int resultnbr1 = 0, resultnbr2 = 0, hasil = 0, prize = 0, result1 = 0, result2 = 0, total = 0, lastIPLogon = 0;
string shop_list = "", opc_list = "", shop_list2 = "", shop_list2_2 = "", zombie_list = "", surgery_list = "", rare_text = "", rainbow_text = "", wolf_list = "";
bool DailyChallenge = false, Carnival = false, Growganoth = false, winterfest_event = false, Valentine = false, Summerfest = false, secret_standopowah = false, restartForUpdate = false, xwhitelist = false, DailyMaths = false, auto_save = false, othersignalcalled = false, wascalled = false;
int Current_Clothes_Need = 7112, NPCX_1 = 0, NPCX_2 = 0, NPCX_3 = 0, NPCX_4 = 0, NPCX_5 = 0, NPCX_6 = 0, NPCX_7 = 0, NPCX_8 = 0, NPCX_9 = 0, NPCX_10 = 0, NPCY_1 = 0, NPCY_2 = 0, NPCY_3 = 0, NPCY_4 = 0, NPCY_5 = 0, NPCY_6 = 0, NPCY_7 = 0, NPCY_8 = 0, NPCY_9 = 0, NPCY_10 = 0;
std::string dailyc_name = "", top_dailyc_list = "";
std::vector<std::pair<long long int, std::string>> top_dailyc;
...
struct Edit_ItemV2 {
	...
	int ID = 0, Break_Hits = 0, Far_Punch = 0, Punch_Place = 0, Punch_Hit = 0, Punch_Id = 0, Gems = 0, Xp = 0, Bonus = 0, Item_Price = 0;
	bool property_unobtainable = false, property_gacha = false, property_untradeable = false, property_blocked = false;
};
...
public:
	...
	std::vector<std::string> get_all_array() {
		...
		for (int i = 0; i < m_data.size(); i++) ret.push_back(fmt::format("[{}]: {}", i, m_data[i]));
		return ret;
	}
...
inline vector<string> explode(const string& delimiter, const string& str) {
	vector<string> arr;
	std::size_t strleng = str.length();
	std::size_t delleng = delimiter.length();
	if (delleng == 0)
		return arr;
...
int items_dat() {
	...
	for (int i = 0; i < itemCount; i++) {
		...
		if (def.actionType == 12 or def.actionType == 45 or def.actionType == 32 or def.actionType == 31 or def.actionType == 122 or def.actionType == 36 or def.actionType == 18) {
		...
		if (itemsdatVersion >= 16) {
			int16_t strLen_ = *(int16_t*)&data[memPos];
			memPos += 2 + strLen_;
		}
		...
		if (def.actionType == 127 || def.actionType == 126 || def.actionType == 118 || def.actionType == 117 || def.actionType == 116 || def.actionType == 115 || def.actionType == 113 || def.actionType == 109 || def.actionType == 106 || def.actionType == 105 || def.actionType == 104 || def.actionType == 102 || def.actionType == 99 || def.actionType == 96|| def.actionType == 86 || def.actionType == 79 || def.actionType == 78 || def.actionType == 75|| def.actionType == 73 || def.actionType == 72 || def.actionType == 71 || def.actionType == 68  || def.actionType == 66 || def.actionType == 65 || def.actionType == 57 || def.actionType == 53 || def.actionType == 52 || def.actionType == 50 || def.actionType == 43 || def.actionType == 40 || def.actionType == 91 || def.id == 5818 || def.id == 5820 || def.id == 6280) def.blocked_place = true;
	...
	if (file_info.is_open()) {
		...
		file_info.close();
		{
			...
			file_info.close();
			{
				ifstream file_info("db/pet_abilities.txt");
				if (file_info.is_open()) {
					string line_info;
					while (getline(file_info, line_info)) {
